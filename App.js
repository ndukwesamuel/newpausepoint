...
export const UpdateScreen = ({ message }) => {
  useEffect(() => {
    console.log("┌─────────────────────────────────────────");
    console.log("│ [UPDATE SCREEN] Displaying Force Update Screen");
    console.log(`│ Platform : ${Platform.OS}`);
    console.log(`│ Message  : ${message || "Default message being rendered"}`);
    console.log("└─────────────────────────────────────────");
  }, [message]);
  ...
  const handleUpdate = () => {
    ...
    console.log("┌─────────────────────────────────────────");
    console.log("│ [UPDATE SCREEN] User tapped 'Update Now'");
    console.log(`│ Opening Store URL: ${url}`);
    console.log("└─────────────────────────────────────────");

    Linking.openURL(url).catch((err) =>
      console.error("[UPDATE SCREEN] Failed to open store link:", err),
    );
  };
...
// ── Update Checker Hook with Complete Visual Logs ───────────────────────────
export const useUpdateChecker = (checkInterval = 60000) => {
  ...
  const checkForUpdates = async () => {
    console.log("┌─────────────────────────────────────────");
    console.log("│ [UPDATE] Initiating Version Check");
    console.log(`│ Local App Version : ${version || "NOT_FOUND"}`);
    console.log(`│ Base API URL      : ${API_BASEURL}`);
    ...
    try {
      ...
      console.log(`[UPDATE] Backend Response HTTP Status: ${response.status} (${response.statusText})`);

      if (!response.ok) {
        console.warn(`[UPDATE] Request completed with non-200 status code: ${response.status}`);
      }

      const data = await response.json();

      const clientVer = data?.clientVersion ?? version;
      const serverVer = data?.currentVersion;
      const isOutdated = clientVer < serverVer;

      console.log("┌─────────────────────────────────────────");
      console.log("│ [UPDATE] Payload Received from Backend");
      console.log(`│ Server Echoed Client Version : ${clientVer}`);
      console.log(`│ Latest Server Version        : ${serverVer}`);
      console.log(`│ Message from Server          : "${data?.message || "N/A"}"`);
      console.log(`│ Full Raw Payload             : ${JSON.stringify(data)}`);
      console.log(`│ Force Update Condition Met?  : ${isOutdated ? "YES 🚨 (BLOCKING APP)" : "NO ✅ (APP OPERATIONAL)"}`);
      console.log("└─────────────────────────────────────────");

      setUpdateInfo((prev) => {
        const prevForce = prev?.clientVersion < prev?.currentVersion;
        const newForce = data?.clientVersion < data?.currentVersion;

        if (prevForce === newForce) {
          console.log("[UPDATE] State evaluation: forceUpdate status unchanged. Skipping state trigger.");
          return prev;
        }

        console.log(`[UPDATE] State evaluation: forceUpdate changed (${prevForce} -> ${newForce}). Updating state.`);
        return data;
      });
    } catch (error) {
      console.error("┌─────────────────────────────────────────");
      console.error("│ [UPDATE] Error Fetching Update Status");
