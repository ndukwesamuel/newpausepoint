...
// ── Notification handler — top level, runs once ─────────────────────────────
...
export const NavigationScreen = () => {
  ...
  const MainScreen = ({}) => {
    ...
    // ── Notification listeners for logged-in user ─────────────────────────────
    useEffect(() => {
...
export function AppNotification() {
  ...
  useEffect(() => {
    ...
    const appStateSubscription = AppState.addEventListener(
      "change",
      (nextAppState) => {
        console.log(`[NOTIF] App state changed: ${appState.current} → ${nextAppState}`);
...
async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    if (finalStatus !== "granted") {
      console.error("[NOTIF] Permission NOT granted — cannot get push token");
      Alert.alert(
