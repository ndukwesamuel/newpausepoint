...
// ── Fix: only update state when forceUpdate actually changes ─────────────────
export const useUpdateChecker = (checkInterval = 60000) => {
  const [updateInfo, setUpdateInfo] = useState(null);
  const version = Constants.expoConfig?.version;
  ...
  const checkForUpdates = async () => {
    try {
      ...
      setUpdateInfo((prev) => {
        ...
        // Only update state if forceUpdate status changed — prevents remount
        if (prevForce === newForce) return prev;
        return data;
      });
    } catch (error) {
      console.error("Error checking for updates:", error);
