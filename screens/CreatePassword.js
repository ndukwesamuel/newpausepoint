...
import React, { useState } from "react";
import AppScreen from "../components/shared/AppScreen";
import { Forminput, Forminputpassword } from "../components/shared/InputForm";
import { MaterialCommunityIcons } from "@expo/vector-icons";
import { useDispatch, useSelector } from "react-redux";
import Toast from "react-native-toast-message";
import { authScreenChange } from "../Redux/OnboardingSlice";
...
const CreatePassword = () => {
  ...
  const resetPasswordMutation = usePublicMutateData(
  ...
  const handleSubmit = () => {
    ...
    // ✅ Call mutation with reset data
    resetPasswordMutation.mutate(
      {
        email: otpemail,
        otp,
        passoword: newPassword, // Note: Backend expects "passoword" (typo in backend?)
        password_confirmation: confirmPassword,
      },
      {
        onSuccess: (data) => {
          ...
          Toast.show({
            ...
            text2: data?.data || "You can now login with your new password",
          });

          // Clear OTP email from Redux
          dispatch(setOtpEmail(null));
