...
//   } = useFetchData_v2("api/v1/visitor/multiEntry/my", "userMultiGuests");

//   // ── Active data based on tab ──────────────────────────────────────────────
//   const singleInvites = guestData?.userInvites || [];
//   const multiInvites = multiData?.data || [];

//   const isLoading = activeTab === "single" ? isLoadingGuests : isLoadingMulti;
//   const isError = activeTab === "single" ? isErrorGuests : isErrorMulti;
//   const error = activeTab === "single" ? errorGuests : errorMulti;
//   const refetch = activeTab === "single" ? refetchGuests : refetchMulti;

//   const filteredSingle = singleInvites.filter((item) =>
//     item.visitor_name?.toLowerCase().includes(searchQuery?.toLowerCase()),
//   );
//   const filteredMulti = multiInvites.filter((item) =>
//     item.visitor_name?.toLowerCase().includes(searchQuery?.toLowerCase()),
//   );

//   const activeFilteredData =
//     activeTab === "single" ? filteredSingle : filteredMulti;
...
//   // ── Insert ads every 3 items (single entry only) ──────────────────────────
//   const getDataWithAds = (data) => {
//     if (!data || data.length === 0) return [];
//     const result = [];
//     let adIndex = 0;
//     data.forEach((guest, index) => {
//       result.push({ type: "guest", data: guest });
//       if ((index + 1) % 3 === 0 && adIndex < guestAdvertisements.length) {
//         result.push({
//           type: "ad",
//           data: guestAdvertisements[adIndex % guestAdvertisements.length],
...
//   // ── Single entry card (existing design) ──────────────────────────────────
//   const SingleHistoryItem = ({ itemdata }) => (
//     <TouchableOpacity
//       style={styles.guestCard}
//       onPress={() => navigation.navigate("guestsdetail", { itemdata })}
//     >
//       <View style={styles.guestIconContainer}>
//         <MaterialCommunityIcons name="account" size={32} color="#3B82F6" />
//       </View>
...
//         <View style={styles.guestTopRow}>
//           <View style={styles.guestInfoBlock}>
//             <Text style={styles.guestPrimaryText}>{itemdata?.access_code}</Text>
//             <Text style={styles.guestSecondaryText}>Access Code</Text>
//           </View>
//           <View style={styles.statusBadge}>
//             <Text style={styles.statusBadgeText}>
//               {(itemdata?.status || "pending").toUpperCase()}
//             </Text>
//           </View>
//         </View>
...
//   // ── Multi entry card ──────────────────────────────────────────────────────
//   const MultiHistoryItem = ({ itemdata }) => {
...
//         onPress={() => navigation.navigate("guestsdetail", { itemdata })}
//       >
//         {/* Purple icon to distinguish from single entry */}
//         <View
//           style={[styles.guestIconContainer, { backgroundColor: "#EDE9FE" }]}
//         >
//           <MaterialCommunityIcons
...
//                 {itemdata?.access_code}
//               </Text>
//               <Text style={styles.guestSecondaryText}>Multi Entry Code</Text>
//             </View>
//             <View
//               style={[
//                 styles.statusBadge,
...
//           {/* Note */}
//           {itemdata?.note ? (
//             <Text style={styles.noteText}>📝 {itemdata.note}</Text>
//           ) : null}
...
//               />
//               <Text style={styles.guestInfoText}>
//                 {itemdata?.total_visits || 0} visits
//               </Text>
//             </View>
//             <View style={styles.guestInfoItem}>
//               <MaterialCommunityIcons
...
//   const renderItem = ({ item }) => {
//     if (item.type === "ad") return <GuestAdCard ad={item.data} />;
//     if (activeTab === "multi") return <MultiHistoryItem itemdata={item.data} />;
//     return <SingleHistoryItem itemdata={item.data} />;
//   };
...
//                 style={[
//                   styles.tab,
//                   activeTab === "multi" && styles.tabActiveMulti,
//                 ]}
//                 onPress={() => {
//                   setActiveTab("multi");
//                   setSearchQuery("");
...
//                   name="account-reactivate"
//                   size={15}
//                   color={activeTab === "multi" ? "#6366F1" : "#9CA3AF"}
//                   style={{ marginRight: 5 }}
//                 />
//                 <Text
//                   style={[
//                     styles.tabText,
//                     activeTab === "multi" && styles.tabTextActiveMulti,
//                   ]}
//                 >
//                   Multi Entry
//                 </Text>
...
//                     style={[
//                       styles.tabCount,
//                       activeTab === "multi" && styles.tabCountActiveMulti,
//                     ]}
//                   >
//                     <Text
//                       style={[
//                         styles.tabCountText,
//                         activeTab === "multi" && { color: "#6366F1" },
//                       ]}
//                     >
//                       {multiInvites.length}
//                     </Text>
...
//                   {activeTab === "multi" ? "Multi Entry Passes" : "Guest List"}
//                 </Text>
//               </View>
...
//                     {searchQuery ? "No matching guests" : "No guests found"}
//                   </Text>
//                   <Text style={styles.emptyStateSubtitle}>
//                     {searchQuery
//                       ? "Try adjusting your search"
//                       : activeTab === "multi"
//                         ? "Create a multi entry pass for frequent visitors"
//                         : "Invite guests to get started"}
//                   </Text>
//                 </View>
//               ) : (
//                 <FlatList
//                   data={getDataWithAds(activeFilteredData)}
//                   keyExtractor={(item, index) =>
//                     item.type === "ad"
//                       ? `ad-${item.data.id}-${index}`
//                       : `guest-${item.data._id || index}`
...
//               style={[
//                 styles.fab,
//                 activeTab === "multi" && { backgroundColor: "#6366F1" },
//               ]}
//               onPress={() => navigation.navigate("inviteguest")}
//             >
//               <MaterialCommunityIcons name="plus" size={28} color="#FFFFFF" />
...
//   tabActiveMulti: { backgroundColor: "#EDE9FE" },
//   tabText: {
//     fontSize: 13,
//     fontWeight: "600",
//     color: "#9CA3AF",
...
//   tabTextActiveMulti: { color: "#6366F1" },
//   tabCount: {
//     marginLeft: 6,
...
//     paddingVertical: 2,
//   },
//   tabCountActive: { backgroundColor: "#D1FAE5" },
//   tabCountActiveMulti: { backgroundColor: "#EDE9FE" },
//   tabCountText: { fontSize: 11, fontWeight: "700", color: "#9CA3AF" },
...
//     letterSpacing: 0.5,
//   },
//   guestNameRow: { flexDirection: "row", alignItems: "center", marginBottom: 8 },
//   guestNameText: { fontSize: 14, fontWeight: "600", color: "#374151" },
//   noteText: { fontSize: 12, color: "#6B7280", marginBottom: 6 },
//   guestBottomRow: { flexDirection: "row", justifyContent: "space-between" },
//   guestInfoItem: { flexDirection: "row", alignItems: "center" },
//   guestInfoText: { fontSize: 12, fontWeight: "500", color: "#6B7280" },

//   // ── Empty ──
//   emptyStateContainer: { alignItems: "center", paddingVertical: 40 },
//   emptyStateAnimation: { width: 160, height: 160 },
//   emptyStateTitle: {
//     fontSize: 18,
//     fontWeight: "700",
//     color: "#111827",
...
const Guests = () => {
  ...
  console.log({
    iioioi: get_user_profile_data?.data?.currentClanMeeting?._id,
  });

  // ── ONE fetch — same endpoint ─────────────────────────────────────────────
  const {
    data: guestData,
    isLoading: isLoadingGuests,
    isError: isErrorGuests,
  ...
  // ── Active data based on tab + search ────────────────────────────────────
  const activeData = (
    activeTab === "single" ? singleInvites : multiInvites
  ).filter((item) =>
    item.visitor_name?.toLowerCase().includes(searchQuery?.toLowerCase()),
  );
  ...
  // ── Single entry card (existing design — unchanged) ───────────────────────
  const SingleHistoryItem = ({ itemdata }) => (
    <TouchableOpacity
      style={styles.guestCard}
      onPress={() => navigation.navigate("guestsdetail", { itemdata })}
          ...
          <View style={styles.container}>
            ...
            <View style={styles.guestListCard}>
              ...
              {activeData.length === 0 ? (
                ...
                <FlatList
                  data={getDataWithAds(activeData)}
                  keyExtractor={(item, index) =>
                    item.type === "ad"
                      ? `ad-${item.data.id}-${index}`
                      : `guest-${item.data._id || index}`
