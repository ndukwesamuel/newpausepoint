  ...
  const emergencyMutation = useMutateData_v2(
    ...
    ["visitors", "invites"],
    {
      onSuccess: (response) => {
        Toast.show({
          type: "success",
          text1: "Emergency invite created — valid for 3 hours",
        });
        queryClient.invalidateQueries({ queryKey: ["userGuests"] });
        refetchGuests();

        // Created invite may come back at data, data.invitation, or data.userInvites
        const createdInvite =
          response?.data?.invitation ||
          response?.data?.userInvites ||
          response?.data;

        if (createdInvite?._id) {
          navigation.navigate("guestsdetail", { itemdata: createdInvite });
        }
      },
      onError: (error) => {
        Toast.show({
          type: "error",
          text1: error?.data?.message || "Failed to create emergency invite",
        });
      },
    },
  );

  const handleEmergencyInvite = () => {
    const now = new Date();
    const threeHoursLater = new Date(now.getTime() + 3 * 60 * 60 * 1000);

    const payload = {
      arraval: now,
      expires: threeHoursLater,
      visitor_name: "Emergency Visitor",
      gender: "Male",
      phone_number: "0000000000",
      location: "Emergency Entry",
    };

    emergencyMutation.mutate(payload);
  };
